...
Player.prototype.checkPack = function() {
  console.log(
    `${this.name} currently has these items in the pack: ${this.getPack()
      .map(function(e) {
        return e.name;
      })
      .join(", ")}`
...
Player.prototype.takeItem = function(item) {
  if (this.getPack().length < 3) {
    console.log(`${Player.name} placed ${item.name} into pack`);
...
Player.prototype.discardItem = function(item) {
  ...
  if (itemIndex === -1) {
    ...
    console.log(`${this.name} has removed ${item} from the pack.`);
    return true;
  }
};
...
Player.prototype.equippedWith = function() {
  if (this.equipped === false) {
    console.log(`${Player.name} had nothing equipped.`);
    return false;
  } else {
    console.log(`${Player.name} has ${this.equipped.name} equipped`);
